...
import { db } from "@/db";
import { apps, volumes } from "@/db/schema";
...
volumesHono.get("/:app_name/volumes", async (c) => {
    ...
    const { summary } = c.req.query() as paths["/apps/{app_name}/volumes"]["get"]["parameters"]["query"];
...
volumesHono.post("/:app_name/volumes", async (c) => {
    ...
    try {
        ...
        const volumeResponse = {
            id: volumeId,
            name: newVolume[0].name,
            region: newVolume[0].region,
            size_gb: newVolume[0].sizeGb,
            state: newVolume[0].state,
            encrypted: newVolume[0].encrypted,
            created_at: newVolume[0].createdAt,
            attached_alloc_id: null,
            attached_machine_id: null,
            block_size: 4096,
            blocks: Math.floor((newVolume[0].sizeGb * 1024 * 1024 * 1024) / 4096),
            blocks_avail: Math.floor((newVolume[0].sizeGb * 1024 * 1024 * 1024) / 4096),
            blocks_free: Math.floor((newVolume[0].sizeGb * 1024 * 1024 * 1024) / 4096),
            fstype: newVolume[0].fstype,
        };
        ...
        return c.json({ error: "Failed to create volume", details: String(error) }, 500);
    }
});
...
// Update volume
volumesHono.post("/:app_name/volumes/:volume_id", async (c) => {
    const { app_name, volume_id } = c.req.param();
    const updateRequest = await c.req.json() as components["schemas"]["UpdateVolumeRequest"];
    ...
    try {
        // For now, we'll only support updating the snapshot retention
        // Real volume resizing would require more complex logic
        ...
        await db
            ...
            .where((table, { eq }) => eq(table.id, volume_id));
        ...
        return c.json({ error: "Failed to update volume", details: String(error) }, 500);
    }
});
...
volumesHono.delete("/:app_name/volumes/:volume_id", async (c) => {
    ...
    try {
        ...
        await db.delete(volumes).where((table, { eq }) => eq(table.id, volume_id));
        ...
        return c.json({ error: "Failed to delete volume", details: String(error) }, 500);
    }
});
...
volumesHono.post("/:app_name/volumes/:volume_id/extend", async (c) => {
    ...
    try {
        ...
        const volumeResponse = {
            id: updatedVolume[0].id,
            name: updatedVolume[0].name,
            region: updatedVolume[0].region,
            size_gb: updatedVolume[0].sizeGb,
            state: updatedVolume[0].state,
            encrypted: updatedVolume[0].encrypted,
            created_at: updatedVolume[0].createdAt,
            attached_alloc_id: null,
            attached_machine_id: null,
            block_size: 4096,
            blocks: Math.floor((updatedVolume[0].sizeGb * 1024 * 1024 * 1024) / 4096),
            blocks_avail: Math.floor((updatedVolume[0].sizeGb * 1024 * 1024 * 1024) / 4096),
            blocks_free: Math.floor((updatedVolume[0].sizeGb * 1024 * 1024 * 1024) / 4096),
            fstype: updatedVolume[0].fstype,
        };
        ...
        logger.error(`Failed to extend volume: ${error}`);
        return c.json({ error: "Failed to extend volume", details: String(error) }, 500);
    }
});